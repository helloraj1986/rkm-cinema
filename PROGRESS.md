...
- ⚠ **Deploy (RKM-HP):** `.\\bootstrap.ps1` → eyeball Home hero/CW rail, Movies/TV grids, an item page (movie + series episodes), Watchlist, top-bar ⌘K search → then **merge `feat/premium-ux-redesign` → main + push** (also still pending for the similar/CW branch + the legacy-removal/restructure deploy). Sandbox gotchas banked in the skill ref (`vite --host 0.0.0.0`, 9p stale-vite restarts, orphan vite squatters on :5173, playwright-core + system Chrome recipe).
