...
## ▶ DEPLOY PENDING on RKM-HP — Plex-style UI revamp (backend `4831da9` + frontend `f2dc060`)
**Cards now open a Plex-style detail ("preplay") overlay.** Everything below was verified headless in the sandbox against the real bundled Jellyfin; the container run is user-side.
1. `cd D:\hermes_agent\hermes-workspace\projects\rkm-cinema`
2. `.\bootstrap.ps1`  (api + web images rebuild — new `/api/jellyfin/detail` + `/person` routes AND the new React UI)
3. On **http://localhost:8124/library**: click any card → detail overlay (backdrop hero, poster, synopsis, ★ rating, genres, studio, cast headshots, big **Resume (n%)**/Play). Series → season-grouped episodes with per-episode Resume %; series-level button auto-continues (Resume S1E1 in the live library).
4. Confirm Play/Resume starts the HLS player at the right position; Esc / ✕ / backdrop close.
5. Expected live values (bundled library, 2026-09-06): (500) Days of Summer Resume (51%) ★7.3 AU-M; 3 Body Problem ★7.5 AU-MA 15+ Netflix, 8 episodes, per-episode resume 16/62/43/44/12/6%.
