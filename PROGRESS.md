...
## ▶ NEXT SESSION — Plex-style HLS/MSE player (DECIDED — plan: `docs/HLS_PLAYER_PLAN.md`)
**Stop patching the progressive/restart-seek player.** After redeploying `f74ceba` the user confirmed the seek bug still affects every episode of 3 Body Problem (and movies): UI seeks but actual playback position doesn't move. Root cause is the transport model, not UI events: we ask the browser to seek server-generated progressive streams. **Plex/Jellyfin-web solve this with HLS+MSE** — the server is asked for the segment at the clicked time; direct-play is used only for browser-seekable MP4.

1. **Phase 0 (½ day)** — probe one 3BP episode's playback-info facts (container/codecs); probe Jellyfin live HLS output (`/Videos/{id}/master.m3u8`, remux-copy vs transcode, URI shapes) to define proxy rewriting. Harness recipe + live-Jellyfin gotchas in the plan doc §6.
2. **Phase 1 (½–1 day)** — `api/routes/jellyfin_hls.py` same-origin HLS proxy (master + media + segments, URI rewriting, additive contract, snapshot/types regen).
3. **Phase 2 (1–2 days)** — `hls.js` engine in `Player.tsx` (native HLS on Safari); keep native direct play for MP4; **delete** `baseRef`/`startAt`/restart-seek offset machinery — position = `video.currentTime`; seek = plain currentTime set; subs overlay trivially aligned; error ladder remux-HLS → transcode-HLS; chip stays.
4. **Phase 3 (½ day)** — headless harness verify (click 10:05 → currentTime ≈ 605 s) then user live test on RKM-HP.
