...
- ▶ **NEXT (approved): Plex-style UI revamp — cards + detail/preplay screen.** Plan: `docs/PLEX_UI_PLAN.md` (created 2026-09-06, approved by user — "create the md file and put its reference in PROGRESS.md so everything can be tracked from one progress file"). Phase 1 = backend detail endpoint; Phase 2 = ItemDetail + MediaCard revamp; Phase 3 = verify/ship. Parked: Bazarr subtitles-for-missing-subs (user: "lets park it for later").

## ▶ NEXT SESSION — Plex-style UI revamp (approved — plan: `docs/PLEX_UI_PLAN.md`)
**Cards + click-to-detail preplay, like Plex. Jellyfin already exposes Overview/Genres/People/Ratings/Studios/backdrops (live-probed) → one additive `/api/jellyfin/detail` endpoint unlocks ~90% of it.**
1. **Phase 1 (½ day)** — `JellyfinLibraryProvider.item_detail(id)` + additive `GET /api/jellyfin/detail?id=`; person-image proxy check; snapshot/types regen; provider+route tests.
2. **Phase 2 (1–1½ days)** — `ItemDetail.tsx` preplay overlay (backdrop hero, poster, synopsis, rating/genres/studio/cast, big Play/Resume, episodes for series) + `MediaCard` revamp (hover play, click→detail, Plex badges). No new npm deps.
3. **Phase 3 (½ day)** — headless verify → commit → `.\\bootstrap.ps1` → user check.
Parked: Bazarr auto-subtitles for files with none (needs OpenSubtitles account; user deferred).
