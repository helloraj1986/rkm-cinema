...
> Last updated: 2026-09-10 (**SECOND MEDIA DRIVE SUPPORTED — branch `feat/configurable-media-libraries`, commits `a402ee8`+`e101581`+`52a168c`+`683415f` (plus `d629e17` media-root wiring): `RKM_MEDIA_PATH` → `/data`, `RKM_MEDIA_PATH_2` → `/media2` (movies on D:, TV on B:), host-path libraries on any root, auto-discovery per root, cross-platform `is_real_media_root()` fix; **431 pytest · ruff · vitest 160/160 · tsc · build**; deploy + verification PENDING, merge PARKED at user request — see LATEST SESSION record. Previous headline: **MEDIA ROOT WIRED TO THE REAL DRIVE — branch `feat/configurable-media-libraries`, commit `d629e17`: `RKM_MEDIA_PATH=D:/RKM_MEDIA`, host-path libraries + TYPE hints, provisioner auto-discovery, fresh watchlist at `<media root>/rkm/`; 398 pytest · vitest 160/160 · tsc · build; deploy note in LATEST SESSION record. Previous headline: **CONFIGURABLE MEDIA LIBRARIES EXECUTED — branch `feat/configurable-media-libraries`, 6 commits `33e46e6`→`0cb8624`; sidebar Libraries group from MEDIA_LIBRARY_N_NAME/PATH, folder-scoped pages, contract 37→39; 366 pytest · vitest 160/160 · tsc · build, live-verified vs bundled Jellyfin; see LATEST SESSION record. Previous headline: **WATCHCARD MORE-MENU CLEANUP + INTELLIGENT SEARCH MERGED TO MAIN — `feat/intelligent-search` (9 commits `b79a2b3`→`3920372`) FF'd to main, `experiment/bundled-docker-stack` FF'd to match, all pushed; vitest 161/161 · tsc · build. Previous headline: **INTELLIGENT GLOBAL SEARCH EXECUTED — branch `feat/intelligent-search`, 8 commits; backend +37th additive path, vitest 158/158 · 329 pytest · tsc · build; see LATEST SESSION record. Previous headline: **PREMIUM PLAYER CHROME EXECUTED — branch `feat/player-tail` (commits `6cdd5c0`+`ae67e03`): Plex-style settings overlay, TV prev/next + S1E4 context, typography coherence; vitest 151/151 · tsc · build. Previous headline: **PLAYER ROADMAP TAIL EXECUTED — branch `feat/player-tail`, 4 commits; frontend-only, vitest 148/148 · tsc · build; see LATEST SESSION record. Previous headline: **SEARCH DETAIL BUG FIXED — live TMDB hit always opened Sappho's Tale; frontend-only, vitest 131/131; see LATEST SESSION record above the next. Previous headline: **UX POLISH PASS EXECUTED — branch `feat/ux-polish-pass`, 7 commits `37d6a5d`→`35025e5`; frontend-only polish + one /api search fix; **vitest 130/130 (was 120) · tsc · build**, headless-accepted live vs the real bundled Jellyfin (0 console errors); full record below. Premium polish on the remaining surfaces (Suggest/Search detail modals, SuggestCard, Settings — new shared a11y Dialog shell), player chrome (§68), phase-9 empty/error/a11y pass, folder URL state + scroll restoration (§60/§63), 8 honest sorts + compact list view (§16–17), card ⋯ context menus + item-page More actions (§46/§25) — plus the empty-live-TMDB-search bug ROOT-CAUSED + FIXED (route never set required SearchResult imdbId/snippet fields → Pydantic error on EVERY live row → silently empty tmdb; now via the shared TMDBService client + logged failures; end-to-end verified `q=fauda` → 2 live rows). ⚠ Redeploy `.\\bootstrap.ps1` on RKM-HP → eyeball (checklist in the record), then merge feat branch → main + push + fast-forward `experiment/bundled-docker-stack`. Previous: **Premium UX redesign EXECUTED — branch `feat/premium-ux-redesign`, 7 commits, frontend-only, vitest 120/120 · tsc · build, live-accepted in real Chromium vs the `:8124` backend; full record below. Previous:** Similar Titles + Continue-Watching Episodes EXECUTED — `feat/similar-titles-cw-episodes` @ `47e3a3c`; 4 commits `8b43efe`/`f69a020`/`7944a38`/`47e3a3c`. Item 4 v2 "Because you watched": additive `GET /api/jellyfin/similar` (contract 35→36; Jellyfin `ProviderIds.Tmdb` → TMDB `/movie|tv/{id}/similar` server-side, cached 6h) + "Because you watched <title>" row on movie/tv item pages (reuses the Suggest card + detail modal → Add/Download work; client-side dedupe vs the library cache). CW episodes gap CLOSED (Option A, contract zero-diff): `continue_watching` reads Jellyfin `/Items/Resume` first → in-progress EPISODES surface (kind=episode + `episode` facet; Resume rows + scan-filter fallback, dedupe by id; played excluded); Home cards show SxEy badge + series name; hover ▶ resumes the episode instantly, card click opens the series page. **311 pytest (+19)** · ruff clean · **vitest 116/116 (+11)** · tsc · build. Headless-accepted LIVE (real bundled Jellyfin 10.11.11): Prisoners + 3BP pages render 10 real similar cards w/ posters; Home CW row = 12 items (5 movies + 7 episodes S1E1–S1E7); episode card click → series page w/ Episodes + Season 1 + similar row; 0 console errors. ⚠ Deploy `.\\bootstrap.ps1` on RKM-HP → eyeball both → merge feat branch → main + push + fast-forward `experiment/bundled-docker-stack`.) Previous: 2026-09-08 (**NEXT: Similar Titles + CW Episodes — branch `feat/similar-titles-cw-episodes` created from main @ `8bb66d9`; plans `docs/SIMILAR_TITLES_PLAN.md` + `docs/CONTINUE_WATCHING_EPISODES_PLAN.md` committed READY, NOT executed. Item 4 v2 \"Because you watched\" (server-side TMDB similar via additive `/api/jellyfin/similar`; TMDB movie/tv similar endpoints live-verified) + Continue Watching episodes gap (Jellyfin `/Items/Resume` lists in-progress episodes; plan Phase 0 probes the bundled stack, Option A keeps the free-form endpoint → no contract change). Execute next session, gates green after every phase.) Previous: 2026-09-08 (**legacy vanilla app REMOVED + restructure MERGED — `refactor/production-repo-structure` merged to main + `experiment/bundled-docker-stack` fast-forwarded (both pushed @ `c43b229`). Then branch `chore/remove-legacy-app`: deleted `frontend/legacy/` (app.js/app.css/api.js/index.html + 4 node harnesses) + the `/legacy` serving surface (nginx alias, compose mount, Sidebar link, LegacyPlaceholder/PortedPlaceholder, `VITE_ENABLE_REACT` flag + build arg) and archived `rebuild_dashboard.py` → `tools/archive/` with its rebuild steps dropped from the ops scripts. React shell is the only UI; Plex backend support + frozen `/api` kept. 292 pytest · ruff · tsc · vitest 105/105 · build · contract zero-diff. ⚠ Deploy `.\\bootstrap.ps1`, then merge + push the cleanup branch.) Previous: 2026-09-08 (**repo restructure EXECUTED — branch `refactor/production-repo-structure`; plan `docs/REPO_STRUCTURE_PLAN.md` Phases 0–5 done, gates green every phase. Monorepo: `backend/` (FastAPI + tests + Dockerfile) + `frontend/` (React shell; legacy vanilla app + node harnesses → `frontend/legacy/`; served at `/legacy`) + deploy/infra stays at root; one-off tools → `tools/archive`, stale docs → `docs/archive`. Compose api context `./backend`, web `frontend/Dockerfile`, legacy volume `./frontend/legacy:/legacy-source`; ops scripts __file__-anchored; contract zero-diff; README/ARCHITECTURE/CI/skill ref updated. ⚠ Deploy `.\\bootstrap.ps1` on RKM-HP then eyeball dashboard + one Play path + /legacy; merge to main + push.) Previous: 2026-09-08 (player bug FIXED: VideoBitRate on transcode; `e71eaf0`). Previous: 2026-09-07 (**Bugfix: Suggest Add on bundled stack — JSON store env now points INSIDE the container (`/data/rkm/watchlist.json`) so the atomic `.tmp` save can't ENOENT, plus env-independent default-path fallback (`…` commit). Cards' Add/Download buttons can no longer open the detail overlay (container-click pattern). 291 pytest · vitest 105/105 · tsc · build ok · redeploy `.\\bootstrap.ps1`.) Legacy parity DONE — Discover / Watchlist / Search / Suggest ported from `/legacy/` to the React shell** — backend `2e18f06` + frontend commit (below); plan `docs/LEGACY_PARITY_PLAN.md` EXECUTED. 290 pytest · ruff clean · vitest 105/105 · tsc clean · `VITE_ENABLE_REACT=1` build ok · contract 34 → 35 paths (additive). ⚠ Deploy `.\\bootstrap.ps1` (api + web both changed), then eyeball the four views; record below. Queued next: roadmap item 4 v2 ("Because you watched" — needs server-side TMDB enrichment).) Library & discovery Phases 0–1 DONE (`05f97d1` + `cfb29fb`) — the Movies/TV Shows folders gained a client-side search/genre/sort toolbar over the shared cache; "Because you watched"/similar (v2) remained the roadmap item-4 follow-up. Plex-style views & navigation Phases 0–2 DONE — `34e4dfa` — every title opens in its OWN URL-backed page (`/library/item/:id`) and the left sidebar browses Movies / TV Shows folders like Plex; plan `docs/PLEX_VIEWS_PLAN.md` EXECUTED. ⚠ Deploy `.\\bootstrap.ps1` then click a card → its own page + browse the folders (RKM-HP eyeball = acceptance). Plex-style UI revamp Phases 1–3 (`4831da9`+`f2dc060`) deployed & USER-CONFIRMED on RKM-HP. HLS player COMPLETE — USER-CONFIRMED FIXED on RKM-HP: "YES THE PROGRESS BAR BUG IS FIXED NOW". **Subtitles FIXED follow-up** (`82cb5f7`): VTT URL shape 404'd on Jellyfin 10.11 → live-verified `…/Subtitles/{n}/0/Stream.vtt` + cold-start timeout 12→45 s; **Subtitle index-0 fix** (`16c104e`): Off no longer collides with track 0. Redeploy `.\\bootstrap.ps1` to go live. **re-platform Phases 0,1a,2,3a,3b,item-2,4 DONE + `thumb` bug FIXED + roadmap item 3 (player features) DONE** — backend 259 pytest + ruff; `web/` tsc+build+vitest; React shell validated live against real Jellyfin data. **`/api` frozen → 34 paths** before this session (additive-only, ADR-0001).
...
**User spec: media folders fully configurable via `.env` (`MEDIA_LIBRARY_N_NAME` + `MEDIA_LIBRARY_N_PATH`); the sidebar renders configured library names; clicking a library lists that folder's media; no hardcoded Movies/TV Shows; validate + warn on missing folders. Plan `docs/MEDIA_LIBRARIES_PLAN.md`; executed in gated phases — backend additive-only (ADR-0001), generic MediaLibrary{name, path}, container-aware env rendering.**
...
**User spec (2026-09-10): one intelligent global search answering "do I own this?" first. Plan `docs/GLOBAL_SEARCH_PLAN.md`; executed in gated phases — backend additive-only (ADR-0001), frontend consolidation, no new deps.**
...
**Executed the queued player roadmap tail (PROGRESS 2026-09-09 record) on a NEW branch from main @ `67bb9fd`; plan `docs/PLAYER_TAIL_PLAN.md`; gates green after every phase — frontend-only, no /api change, no new deps.**
...
**Executed both READY plans back-to-back (`docs/SIMILAR_TITLES_PLAN.md` + `docs/CONTINUE_WATCHING_EPISODES_PLAN.md`), gates green after EVERY phase. Headless-accepted live against the real bundled Jellyfin 10.11.11; RKM-HP deploy + eyeball remains the acceptance (then merge → main).**
...
## ▶ LATEST SESSION (2026-09-08) — repo restructure EXECUTED: backend/ + frontend/ monorepo ✅ (plan: `docs/REPO_STRUCTURE_PLAN.md`)
...
## ▶ LATEST SESSION (2026-09-07) — Legacy parity: Discover / Watchlist / Search / Suggest → React ✅ (plan: `docs/LEGACY_PARITY_PLAN.md`)
...
## ▶ LATEST SESSION (2026-09-07) — Library & discovery Phases 0–1 DONE ✅ (roadmap item 4; plan: `docs/LIBRARY_DISCOVERY_PLAN.md`)
...
## ▶ LATEST SESSION (2026-09-07) — Plex-style views & navigation Phases 0–2 DONE ✅ (frontend-only; plan: `docs/PLEX_VIEWS_PLAN.md`)
