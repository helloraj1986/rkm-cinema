...
NON_CONTRACT_MODELS = {
    ...
    # ⚠⚠ THE AUTO-PICK (2026-09-21) — the same trade again: the three routes behind these are new and
    # answer `{}` in the contract (only their REQUEST bodies are schemas), so each model's shape source is
    # the interface the app reads. Registered one by one on purpose: an exemption is a deliberate act.
    "SubtitleAutoPickSettings": {
        "reason": _PLAYER_SHAPE_REASON,
        "shape": ("frontend/src/lib/api/client.ts", "SubtitleAutoPickSettings")},
    "SubtitleAutoFacts": {
        "reason": _PLAYER_SHAPE_REASON,
        "shape": ("frontend/src/lib/api/client.ts", "SubtitleAutoFacts")},
    "SubtitleAutoOutcome": {
        "reason": _PLAYER_SHAPE_REASON,
        "shape": ("frontend/src/lib/api/client.ts", "SubtitleAutoResult")},
    "SubtitleAutoApplied": {
        "reason": _PLAYER_SHAPE_REASON + " This one is INLINE in that interface, so its source is the "
                                         "dotted path `SubtitleAutoResult.applied`.",
        "shape": ("frontend/src/lib/api/client.ts", "SubtitleAutoResult.applied")},
    "SubtitleSettingsResponse": {
        "reason": _PLAYER_SHAPE_REASON,
        "shape": ("frontend/src/lib/api/client.ts", "SubtitleSettingsShape")},
    "ErrorEnvelope": {
