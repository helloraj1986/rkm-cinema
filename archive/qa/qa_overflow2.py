...
async def main():
    async with async_playwright() as p:
        ...
        dev = p.devices["iPhone 13"]  # real device: innerW 390
        page = await browser.new_page(viewport=dev["viewport"], device_scale_factor=2,
        ...
        res = await page.evaluate("""() => {
          ...
          const inScroller = (el) => {
            ...
            while (p) { if (sc(p)) return true; p = p.parentElement; }
            return false;
          };
          const bad = [];
          ...
          // de-dup by selector, keep widest
          const seen = {};
          for (const b of bad) {
            if (!seen[b.el] || b.right > seen[b.el].right) seen[b.el] = b;
          }
