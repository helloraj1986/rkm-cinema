...
│   └── RKMCinemaTV.xcodeproj + RKMCinemaTV/
...
**Xcode project structure:** commit the `.xcodeproj` (solo dev; no generated-project tooling needed).
⚠ If project-file churn ever becomes painful, XcodeGen (`project.yml` → generated `.xcodeproj`) is the
upgrade path — not needed to start. `.gitignore` already excludes `xcuserdata/`, `DerivedData/` and
`.build/`.
...
| Phase | Work | Gate |
|---|---|---|
| **0. iOS shell** | `apple/Shared/` (address) + `apple/ios/`: setup screen, WKWebView, ATS, `allowsInlineMediaPlayback`, unreachable-server state. | Install on the iPad; enter the address; sign in; play a title; sign out to the app's own state. A wrong address offers Change server. |
| **1. tvOS skeleton** | `apple/tvos/`: server-address screen (pre-filled), sign-in, Who's watching — focusable. | Runs on the Apple TV simulator; sign-in + profile switch work on a real Apple TV. |
