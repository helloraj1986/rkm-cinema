...
- **Phase 5 — UI:** Sidebar replaces the hardcoded Browse Movies/TV Shows entries with a **Libraries** group rendered from the folders endpoint (configured names when present, else server folder names; warning glyph on unresolved); router: `/library/folder/:folderId` renders a kind-free folder page (heading = resolved name, folder-scoped items); `/library/movies` + `/library/shows` become aliases that resolve to the matching configured/server library (preserving `?genre=`), so global-search genre links keep working; MobileNav tabs become Home + first two libraries (icon by folder collection type); empty/error states for no folders.
  - ⚠ **REVISED 2026-09-14 — the "first two libraries" half was wrong, and he reported the cost from his iPad:** *"even though raj profile have access to all three libraries..only two can be seen at the bottom"*. The bar now shows **as many library tabs as its measured width allows** (max 4; `frontend/src/app/layout/lib.ts::libraryTabsThatFit`, unit-tested so no tab is ever below the legible minimum), and More carries the rest **plus any library the server could not resolve, with its reason** — the sidebar already did that and the bar did not, so one library was visible-and-explained on a desktop and silently absent on a phone. Both surfaces now share `libraryNavEntries` (`features/library/lib.ts`), so they cannot drift again. Spec §59 carries the same revision.
