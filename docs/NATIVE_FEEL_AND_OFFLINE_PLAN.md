...
|---|---|---|---|
| **E1** | Add a throwaway loopback server to the shell serving one file from the container; load `http://127.0.0.1:<port>/probe.mp4` in a `<video>` inside the page (a one-line `console.log` of `video.canPlayType` + `onerror` through the existing bridge) | **The single most important unknown**: does media play from loopback inside this WKWebView, with seeking (Range)? Also compare a `WKURLSchemeHandler` URL in the same build — one build answers both. | One Mac round (~1 h) · ✅ **ANSWERED 2026-09-16 (B0 spike, `spike/offline-loopback`): PASS** — 206 for `bytes=0-1/1128375` and then for the whole range, `metadata ok 960x540 duration=5.01`, `seek to 2.51 -> ok`, `RESULT play=ok mediaError=none`; ⚠ and the `WKURLSchemeHandler` failed **for real** (`mediaError=code=4` *with* bytes served), so §4.1's rejection of a custom scheme for media is measured too. **⇒ §4.4–4.6 stand as written; `B1` is next.** ⚠ Reminder that the two legs are different networks: the *download* rides the **tailnet** (`rkm-hp…:8124`, ≈3.8 MB/s measured), the *playback* leg is **loopback**, which is the point — a downloaded film must play with Tailscale down |
...
✅ **MEASURED 2026-09-16 — it works, WITH SEEKING** (`spike/offline-loopback`, simulator, `tools/check_spike_e1_e2.py` → PASS: 206 on the wire for `bytes=0-1`, `metadata ok 960x540`, `seek to 2.51 -> ok`, `play=ok`). §4.4–4.6 are no longer assumptions, the `WKURLSchemeHandler` alternative is measured-out rather than rejected on principle, and **`B1` is the phase to start**.
