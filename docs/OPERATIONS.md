...
|---|---|---|
| `.\rkm.ps1 status` | Containers, state volumes, app + Jellyfin health, library counts, scan state | Anything looks off — **start here** |
| `.\rkm.ps1 deploy` | Build + refresh the stack, wire libraries, trigger a scan | You changed `.env`, or code, or want the stack rebuilt |
| `.\rkm.ps1 backup` | Archive Jellyfin state to `D:\RKM_BACKUPS` (keeps newest 7) | Before anything risky |
| `.\rkm.ps1 restore -Archive <file>` | Replace current state from an archive | Something was lost / a bad upgrade |
| `.\rkm.ps1 schedule` | Install the nightly 04:00 backup task | Once, per machine |
| `.\rkm.ps1 diagnose` | Classify every TV series: watched vs episodes present | Shows look watched, or episodes are missing |
| `.\rkm.ps1 logs` | Tail api + web + jellyfin | Right after a deploy that reported problems |
...
2. **Don't restart the stack while a library scan is running** — a cancelled scan leaves shows present with no episodes attached, which reads as "everything watched". `.\rkm.ps1 status` shows scan state.
3. **Run compose with `-p rkm-bundled`** (bootstrap and `rkm.ps1` always do). A different project name creates *fresh empty* volumes and orphans the real ones.
...
|---|---|---|
| Library rows greyed out / disabled | `.\rkm.ps1 status` | App has no Jellyfin credential, or the library path does not resolve — `status` prints the exact warning |
| Every show shows as watched | `.\rkm.ps1 diagnose` | Series with no episodes attached read as "all played" (Jellyfin's own logic). Healthy series print `played=False` |
| Episodes missing under a show | `.\rkm.ps1 diagnose` | Scan mid-flight, or a cancelled scan — check scan state with `status` |
...
| Deploy stopped at the provisioner | `.\rkm.ps1 logs` | Read the `[jellyfin]` line it stopped on — the messages name the cause |
...
| `.\rkm.ps1 deploy` (rebuild) | kept | untouched |
...
| `tools\rkm_status.py` | the deep status behind `.\rkm.ps1 status` |
