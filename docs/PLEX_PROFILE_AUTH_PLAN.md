...
| **Consequence:** the app cannot act as a password-protected profile without its password. The administrator can **reset** it (`POST /Users/Password?userId=`, `ResetPassword: **false**`) but cannot bypass it | same; reset is already implemented (`admin_users.py`). ⚠ This row said `ResetPassword: true` until 2026-09-13 — written before the measurement; `true` is a silent no-op that CLEARS a password (see `ADMIN_CREDENTIALS_PLAN.md` §6c) |
...
| `UpdateUserPassword {CurrentPassword, CurrentPw, NewPw, ResetPassword}` | self-change = `CurrentPw` + `NewPw`; admin reset = `CurrentPw: ""` + **`ResetPassword: false`** (⚠ corrected 2026-09-13: this row said `true`, which is a silent no-op that CLEARS a password — `ADMIN_CREDENTIALS_PLAN.md` §6c) |
...
| **D** | admin extras in `Settings → Household`: rename, "has password" state, and the profile's own "change my password" screen — ⏩ **MOVED 2026-09-12 into `ADMIN_CREDENTIALS_PLAN.md`** (user decision: folded into the "no password in `.env`" workstream, which owns the admin rename and the self-service password screen) | gates + DOM check |
| **E** | enforcement sweep (401/403 for every router; a profile cannot reach admin routes), ADR-0006, docs (`ARCHITECTURE`, `OPERATIONS`, `README`), PROGRESS record — **still OWNED HERE**: the credential workstream does not cover enforcement | full suite + docs links |
