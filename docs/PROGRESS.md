## ▶ ⚠ **APPLE PHASE 0 — HIS XCODE PROJECT LANDED AND AUDITED; FIRST BUILD FAILED ON ONE SETTING (now fixed); DEPLOYMENT TARGET OPEN** (2026-09-14, latest) · branch **`feat/apple-clients`** (tip = `git log --oneline -1`) · his commit **`9d9d5be`** = `apple/ios/RKMCinema.xcodeproj` + **the SHARED scheme** · **his Swift is still uncompiled**
...
**⚠ STILL NOT VERIFIED: the SwiftUI/WebKit code has never met a compiler.** The failed build died in the build graph, before any compile step ran — so this round told us nothing about the code itself. The next build is the first real compile.

**NEXT ACTION (his):** quit Xcode, `git pull`, reopen, then `./apple/scripts/mac-round.sh ios --sim`. ⚠ If it repeats "Multiple commands produce", that is a stale build description — `rm -rf ~/Library/Developer/Xcode/DerivedData/RKMCinema-*`. If `xcodebuild` says the deployment target is outside the supported range, that message names the range: paste it.
