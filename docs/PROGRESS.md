...
4. ⚠⚠ **At least one downloaded title makes the round complete — AND THE REBUILD IS WHAT REMOVES IT.** The synthetic suite passes without one, but the real-film check and the event direction need a title the downloader knows about. ⚠ `mac-round.sh --sim` **REINSTALLS**, and a reinstall gives the app a **new Data container**: the film lands in the old one and the new install starts empty (this is exactly how the 2026-09-16 round reached `count=0` — 16/16 cases green, both commands answered, and nothing to emit). ⇒ **Download a title, then relaunch WITHOUT reinstalling:**
   ```
   ...
   ```
   Downloading is HUD → **offline (B2)** → **Load titles** → **Download** on a SMALL title → wait for the row to say *ready*. ⚠ A plain relaunch keeps the container, so the film is still there — and B2's reconciler re-derives it from disk on the next launch, which is the other half of why that gate could pass at all.
