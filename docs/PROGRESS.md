## ▶ LATEST SESSION (2026-09-12) — SUBTITLES, PHASES 0–3 OF 6 EXECUTED ⏳ IN PROGRESS (branch: `feat/subtitles-opensubtitles`, commits `1db4215` (plan) → `4d08890` (P0) → `e27d28d` (P1) → `c7e4412` (P2) → `c29ce0b` (P3); plan `SUBTITLES_OPENSUBTITLES_PLAN.md`. **Phase 4 next; the user has not deployed any of this yet.**)
**User instruction:** *"proceed with next in rkm-cinema"* — the queued item was the scoped OpenSubtitles work. The branch was cut from an older `main`, so it was rebased onto current `main` (`251ec63`) first and force-pushed with an explicit `--force-with-lease` (only the plan doc + probe were on it). The user then registered an OpenSubtitles API key and pasted it into the repo `.env`, which unblocked the live half of Phases 2–3.

**Phase 0 (`4d08890`) — config plumbing + ONE .env parser.** Five settings declared on `Config`, read in `_load()`, routed into `.rkm.env` (api only), one api startup line, `.env.example` + live `.env` documented.
- **`has_opensubtitles()` keys off the API KEY alone.** A missing login degrades to the anonymous tier (5 downloads/24h per IP), never to "disabled". `validate_required()` untouched — a blank config must boot.
- **`backend/config/env_file.py` is now THE .env parser** for `render_config`, `config.settings` and `tools/rkm_common` (three copies before). The api's copy did a bare `partition("=") + strip()`, so a quoted value arrived WITH its quotes, and none tolerated a leading BOM. Old-vs-new on one quoted+BOM'd line: OLD `['\ufeffOPENSUBTITLES_PASSWORD', …]` → NEW `['OPENSUBTITLES_PASSWORD', …]`; `'p#ss w0rd'` → `p#ss w0rd`. The plan's §4.1 trap was real: its own snippet starts with a BOM.

**Phase 1 (`e27d28d`) — `services/opensubtitles.py` + 59 fake-transport tests** (no live calls in the suite). Search keyed `tmdb_id → imdb_id → title+year(+season/episode)`; empty results are `[]`, not an error; typed taxonomy (`NotConfigured / AuthFailed / QuotaExhausted / RateLimited / Failed`) defined with the client.
- **A `/download` POST is never retried** (it may already have been charged); idempotent GETs, including 5xx, retried exactly once. The download `link` is pre-signed: fetched WITHOUT the `Api-Key`, never returned to a client, never logged.
- **Live:** tmdb_id and imdb_id each return 30 results for the same film; an episode keyed by series title+season/episode returns 19; a `hi` search returns Hindi results; one real download (91,617 bytes, plain `.srt`, UTF-8, LF) proved the byte path.
- ⚠ **Live finding:** `attributes.format` is FREE TEXT, not an extension (`eng-sdh`, `eng-full`, `x265-heteam`, `23`). `format` now comes from `files[0].file_name`; the vendor label is `vendor_format`, display-only.

**Phase 2 (`c7e4412`) — delivery: `services/subtitles.py` + `item_path`/`refresh_item`/`upload_subtitle`/`subtitle_search_context` on the provider.** The only code that writes to the media drive. Sidecar `<stem>.<lang>.srt`, atomic, UTF-8, encoding normalised (utf-8/-sig, utf-16, cp1252); a same-language sidecar is **REUSED** (no download, the user's file untouched); overwriting needs `replace_existing=True`; the **ITEM** is refreshed and a library scan can never be triggered (test-asserted); a non-subtitle payload (HTML behind a 200) is refused; `resolve_active_track()` maps a stored identity to the CURRENT index or to `None` — never to a different subtitle — with a 639-1/639-2 normaliser (`en` vs `eng`; German `ger` ≠ `ge`).
- ⚠ **Live finding 1 — the upload endpoint is JSON, not multipart.** A hand-built multipart post gets **415**; the server's own openapi declares `UploadSubtitleDto` (`Data` = base64). Found by reading the RUNNING server's `/api-docs/openapi.json` after the 415, then re-verified (204 + track appears).
- ⚠ **Live finding 2 — indexing LAGS the delivery.** Reading tracks right after an upload can return the OLD list (the first live run reported `tracks: []` for a subtitle it had just uploaded). `_tracks()` re-reads a bounded ~1 s before accepting an empty answer, and still reports empty honestly if that is the truth.
- **Verified live end-to-end** (item resolved by EXACT tmdb id, never a fuzzy title): the track appeared in `PlaybackInfo` and **the app's own VTT proxy URL served real content** — 119,258 chars of WEBVTT, first cue *"This is a story of boy meets girl."* The **upload fallback** is what ran (the sandbox has no `/data` mount — the fallback's own use case); **the sidecar write path is unit-tested only**, because only the api container can see the media mounts.

**Phase 3 (`c29ce0b`) — `services/subtitle_store.py` + three endpoints + an additive `playback-info` field.** Prefs are per ITEM and keep an IDENTITY (never an index); usage is per SUBTITLE (global, ranks results, drives "Used N times"). One JSON file beside the watchlist, atomic + mtime-cached + corrupt-tolerant. JSON rather than SQLite deliberately (matches the live store, no dependency, no migration). Endpoints: `GET /api/jellyfin/subtitle-search` (local tracks first and unchanged, then ranked remote rows with used_count/last_used/active), `POST /api/jellyfin/subtitle-select` (download + attach + persist + count in one action), `POST /api/jellyfin/subtitle-disable` ("off" keeps the record so re-enabling is one tap). Contract **38 → 41 paths**, snapshot **+183/−0** and typed client **+216/−0**, both purely additive. A vendor failure degrades the SEARCH route only (local tracks still return; playback never blocked).
- **Two real bugs the new tests caught before shipping:** (1) the ranking lookup assumed one key form for the usage map — the store returns `{"os:123": n}` while the raw map is `{"123": n}`, so every count read as 0 and "rank by our usage" silently did nothing (`count_for()` now resolves both); (2) the service built its own client lazily, so a route-level injection was ignored and the TESTS WERE REACHING THE LIVE VENDOR API — the route now constructs the client and injects it.
- **Verified live through the real api** (uvicorn on `:8125`, isolated store in `/tmp`, temp Jellyfin key, all traces removed): movie search → 30 remote results; **episode search → 19 remote AND the item's 2 pre-existing local tracks** (criterion 2 for movies *and* TV); select → `delivered=upload`, `used_count=1`, `preferred_subtitle` resolved to index 0; `playback-info` returns that same preference; a re-search ranks it first with `used=1`; disable → `playback-info` reports null; the JSON store on disk matches the plan's §3.4 shape.
- ⚠ **Refines the Phase-2 quota finding:** `remaining` DID decrement for a NEW file (4 → 3) but not for a repeat download of the SAME file — so it is a genuine daily counter and a repeat download is free. Before the first download the figure is unknown (`None`) because key-only has no `/infos/user`; the UI must stay silent until it has been learned, never invent a number.

**Gates:** **668 pytest, 0 failures** (Phases 0–3 added 137 tests) · ruff clean · `tsc --noEmit` clean · vitest **163/163** · vite build green · docs link check 34 files / 7 links · openapi snapshot and typed client both purely additive. ⚠ Correction to this record's earlier version: it reported `589 passed / 1 failure (test_runtime_loader_merges_runtime_json)`. That failure was an ENVIRONMENT artefact — the sandbox shell exported an EMPTY `JELLYFIN_API_KEY`, which the real-env passthrough applies over `runtime.json`; with that variable gone the suite has been fully green since. No production impact (the container's key comes from `.rkm.env`/`runtime.json`, never a shell), but it IS a narrow real weakness worth a later look: an empty exported var silently overrides `runtime.json`.

**⚠ Nothing here is deployed.** The running stack is built from `main`; this branch needs a build before anything is visible. When Phase 4 lands, the acceptance run is `docker compose -p rkm-bundled up -d --build api web` (api + web both changed) — **not** a full `bootstrap.ps1`, so a library scan is not touched — then the user's eyeball. A `.env`-only change would additionally need a full deploy, because the api's config IS the rendered `.rkm.env`.

**Next — Phase 4 (player UX, plan §3.7):** panel section as Off / local / OpenSubtitles rows with inline "Search OpenSubtitles" (never leaving the player), per-row busy state, toasts for quota/limit errors, active marker, auto-apply on load from `preferred_subtitle`, and pure helpers (`rankSubtitleResults`, `resolveActiveSubtitle`, `usedCountLabel`) unit-tested with vitest, plus `tools/measure_player_layout.py` still 10/10. Then **Phase 5**: hardening (creds blanked / quota exhausted / network down all leave playback, seek and local subs working), docs (`.env.example`, README, ARCHITECTURE, OPERATIONS symptom→command row), **ADR-0005**, and the user's deploy + eyeball. Open decisions still standing from the plan: §10.3 store = JSON sibling file (implemented as the plan recommended; SQLite remains a separate decision) and §10.4 default languages (`en` implemented; add `hi`/`ta` in `.env` if wanted).
