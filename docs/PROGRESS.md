## ▶ LATEST SESSION (2026-09-12) — SUBTITLES, PHASES 0–1 OF 6 EXECUTED ⏳ IN PROGRESS (branch: `feat/subtitles-opensubtitles`, commits `32d0f84` (plan) → `4d08890` (Phase 0) → `e27d28d` (Phase 1); plan `SUBTITLES_OPENSUBTITLES_PLAN.md`. **⚠ BLOCKED-ON-USER for Phase 2: the OpenSubtitles API key.**)
**User instruction:** *"proceed with next in rkm-cinema"* — the queued item was the scoped OpenSubtitles work. The branch was cut from an older `main`; rebased onto current `main` (`251ec63`) first and force-pushed with an explicit `--force-with-lease` (the plan doc + `tools/probe_subtitles.py` only — no code was on it yet).

**Phase 0 (`4d08890`) — config plumbing + ONE .env parser.** Declared the five settings on `Config` (`OPENSUBTITLES_API_KEY/USERNAME/PASSWORD/LANGUAGES/ENABLED`), read them in `_load()`, added `has_opensubtitles()` / `has_opensubtitles_login()` / `opensubtitles_languages()` / `opensubtitles_disabled()`, routed them into `.rkm.env` (api only), added ONE api startup line, and documented `.env.example` + the live `.env` (blank = feature OFF).
- **The rule that matters: `has_opensubtitles()` keys off the API KEY alone.** A missing login degrades to the anonymous tier (5 downloads/24h per IP), never to "disabled" — key-only is a supported mode. `validate_required()` is untouched: subtitles are optional and a blank config must still boot.
- **`backend/config/env_file.py` is now THE .env parser** for `render_config`, `config.settings` AND `tools/rkm_common` (three copies before). The api's copy did a bare `partition("=") + strip()`, so a quoted value arrived WITH its quotes, and none tolerated a leading BOM. Proved old-vs-new on one quoted+BOM'd line: OLD keys `['\ufeffOPENSUBTITLES_PASSWORD', …]` → NEW `['OPENSUBTITLES_PASSWORD', …]`; `'p#ss w0rd'` → `p#ss w0rd`. (The plan's §4.1 trap is real: the spec's own snippet starts with a BOM.)
- Extra render-time line prints whether OpenSubtitles is on and whether a login is configured — the "I pasted the key and nothing happened" failure mode is now visible before the containers start.
- 41 new tests over 3 files; `grep -rin opensubtitles frontend/dist frontend/src` → 0 hits; a dry render (write_env_key stubbed, nothing written) shows all five keys reaching the `.rkm.env` mapping.

**Phase 1 (`e27d28d`) — `backend/services/opensubtitles.py` + 58 unit tests on a scripted fake transport** (no network, no quota). Search keyed `tmdb_id → imdb_id → title+year(+season/episode)`; imdb sent numeric (`tt0133093` → `133093`); empty results are `[]`, not an error. Typed taxonomy: `NotConfigured / AuthFailed / QuotaExhausted / RateLimited(Retry-After) / NoResults / UnsupportedFormat / TransportError`, defined WITH the client.
- **Never hardcode the quota** (the plan's ⚠): `allowed_downloads`/`remaining_downloads` from `/infos/user` and `remaining`/`reset_time_utc` from every `/download` are read at runtime; a 429 is classified from its body as quota-spent vs slow-down.
- **A `/download` POST is never retried** (it may already have been charged — a blind retry spends a second download); idempotent GETs, including 5xx, are retried exactly once. The JWT is fetched lazily BEFORE a download (a bare 401 round-trip would waste a request), cached 12h, one re-login on expiry.
- The download `link` is pre-signed: fetched WITHOUT the `Api-Key`, never returned to a client, never logged (`safe_url()` strips query strings; password/JWT/key pinned out of the logs by test). Bitmap/archive formats are rejected before any bytes are fetched.
- **LIVE-verified cheaply, zero quota spent:** no key → refused locally before any request; a bogus key → the REAL API answered `403 You cannot consume this service` → `AuthFailedError` naming `OPENSUBTITLES_API_KEY`. Base URL, TLS, header names and the mapping are therefore verified against the vendor, not just against a fake.
- Gates: **589 passed / 1 pre-existing failure** (`test_runtime_loader_merges_runtime_json` — this sandbox shell exports an EMPTY `JELLYFIN_API_KEY`, which the real-env passthrough applies over `runtime.json`; it fails IDENTICALLY on `main`, verified by stash. Environmental, no production impact) · ruff clean · docs link check 34 files / 7 links.

**⚠ WHAT THE USER MUST DO (nothing is live yet — the repo is deployed from `main`, and this branch has no key):**
1. Register a free OpenSubtitles API key: **opensubtitles.com → API Consumers → add consumer** (name e.g. `RKM Cinema`). The key identifies the APP; it is mandatory on every call.
2. Paste it into the repo `.env` as `OPENSUBTITLES_API_KEY=…` (the block is already there, blank). Optionally also `OPENSUBTITLES_USERNAME/PASSWORD` — the account login lifts the daily download quota (~10–20/day vs the anonymous 5/24h per IP).
3. **A `.env` edit needs a FULL deploy, not `up -d --build api`** — the api has no `.env` of its own; its config IS the rendered `.rkm.env`. So `.\\rkm-cinema.ps1 status` (let any library scan finish) then `.\\rkm-cinema.ps1 deploy`. Look for `[env] OpenSubtitles: enabled — anonymous (no login)` (or `login configured`) in the render output, and `OpenSubtitles enabled (…) — subtitle search languages: en` in the api log.
4. Phase 0's last two confirmations belong on his box (no docker daemon in the sandbox): `docker compose -p rkm-bundled exec api python -c "from config.settings import get_config as g;c=g();print(c.has_opensubtitles(), bool(c.OPENSUBTITLES_API_KEY))"` → `True True`, and a test write from the api to the media drive (Phase 2 leans on the api's rw mounts).

**Next (Phases 2–5, unchanged order):** 2 `attach_subtitle` + dedupe (sidecar `.srt` beside the media file, atomic write, item refresh — never a library scan) with exactly ONE deliberate live download; 3 `services/subtitle_store.py` + the three routes + the additive `playback-info.preferred_subtitle` (contract 38 → 41 paths, snapshot + typed client regenerated); 4 player panel + auto-apply + pure helpers (stream indices stay POSITIONAL — identity is stored, the index is resolved at playback); 5 hardening + docs + **ADR-0005** + this record's successor.
