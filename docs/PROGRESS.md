## ▶ ✅ **B3 BUILT — THE DEVICE CAN SERVE THE FILM TO ITSELF: a loopback HTTP server on an EPHEMERAL, LOOPBACK-ONLY port answers real byte ranges, and a REPLY-CAPABLE bridge tells the page what it has — ⚠ and, like B2, NOT ONE LINE OF IT HAS RUN ON THE MAC YET** (2026-09-16) · branch **`feat/offline-downloads`** (same branch as B2, on top of `e39252e`) · **NEW** `apple/ios/RKMCinema/Offline/{OfflineHTTP,OfflineBridgeContract,OfflineProbeCases,OfflineServer,OfflineBridge}.swift`, `apple/ios/RKMCinema/Debug/OfflineServerProbe.swift`, **NEW** `tools/check_offline_server.py` (**the gate, 8 selftests**), **NEW** `docs/adr/ADR-0009-offline-loopback-server.md` · changed: `Shell/WebShellView.swift` (⚠ `addScriptMessageHandler(_:contentWorld:name:)` — the reply-capable form, and the whole phase depends on it), `Shell/WebShellModel.swift` (the page-loaded probe trigger), `Shell/WebBridge.swift` (a new `offline` report case — the only witness the native→page direction has), `App/AppModel.swift`, `Debug/DebugHUD.swift` (a `lpb` line), `apple/scripts/{check-offline-core.py,check-apple-typecheck.sh,check-imports.py,typecheck-stubs/Stubs.swift}`, `apple/LOGGING.md` §9, corrected: **`NATIVE_FEEL_AND_OFFLINE_PLAN.md` §4.4/§4.5/§6** · ⚠ **iOS ONLY: no web, no backend, no nginx — no `apply`, and nothing becomes visible until it is built on the Mac**

⚠⚠ **AND B2'S OWN MAC ROUND IS STILL UNANSWERED — START THERE.** The last session handed over one command and its verdict never came back: `python3 tools/check_offline_download.py` (it was re-run after the checker's two bugs were fixed in `e39252e`). ⚠ **Run it FIRST, before the B3 round** — it reads what is already on disk and needs no rebuild. If it says PASS, B2's gate is met and this record's sibling can be marked done; if it says **"stranded manifest"**, the film landed in an earlier install's container, and the fix is the same round below (download one title again).
...
bash apple/scripts/mac-round.sh ios --sim -RKMOfflineServerProbe YES -RKMOfflineBridgeProbe YES
python3 tools/check_offline_server.py
```

1. **`check_offline_download.py` first** — it reads what is already on disk and needs no rebuild (see the warning at the top of this record).
2. **The build + launch** installs and launches on the iPhone simulator with the two probe switches. ⚠ `mac-round.sh` passes them through (it used to drop them silently — fixed in B0 and verified then). On a **device**, launch from Xcode and set the same two switches as scheme arguments.
...
4. **At least one downloaded title makes the round complete.** The synthetic suite passes without one, but the real-film check and the event direction need a title the downloader knows about. ⚠ If the reinstall moved the container, download one again (HUD → **offline (B2)** → **Load titles** → **Download** a SMALL title), then relaunch with the probe arguments.
