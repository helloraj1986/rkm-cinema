...
**GATES — ALL GREEN:** `pytest 1163/1163` · `ruff check` clean · `check_md_links.py` clean (54 files) · `openapi.v1.json` regenerated (58 paths) · and ⚠ **15/15 FALSIFICATIONS RED** (`/root/falsify_b1.py` — every rule reverted one at a time and the named test required to fail: idempotency, the borrowed file, the range clamp, the 416, the pre-check cap, the in-flight ceiling, failed-vs-missing, the stall rule, the sweep, the HEVC/10-bit rung, the path sanitiser, the unknown-mode refusal, the not-ready guards, the 410, and the explicit HEAD route). **Two of those fifteen did not go red on the first attempt** — `artefact()` carries *two* independent guards (state ≠ ready, and size ≤ 0), so a single-line revert was masked by the other, and the test itself was too weak to distinguish `409` from `410`. Both the mutation and the test were strengthened rather than accepting a pin that proves nothing. **Idempotency is proved by COUNTING UPSTREAM CALLS** — a second `prepare` that quietly re-transcoded the film would also read `state: ready`.

⚠⚠ **HIS STEP — ONE COMMAND, and it is the only one** (PowerShell 5.1: one command per line). ⚠ **No `git pull`: the sandbox and your Windows box are the SAME working tree, and it is left checked out on `feat/offline-api` for exactly this** — a branch switch here moves your checkout too.
...
⚠ **WHAT IS *NOT* VERIFIED, stated plainly:** anything on the running stack. This gate is `pytest` + falsification in the sandbox; **no title has been packaged from your real Jellyfin**, Transcode rungs (remux / transcode_audio / transcode) have **never run against a real film**, **`HEAD` is untested through nginx**, and **the device half does not exist yet** (B2). ⚠ One honest limit recorded in the ADR: the TTL is swept **lazily at `prepare`**, not by a timer — stale bytes cannot block a new download, but a household that never downloads again keeps its files. Another: a **retry restarts a packaging job from zero** (the resumable leg is the *device's* download, which is the leg that crosses a network).

⚠ **STILL OPEN, unchanged:** ⚠ **`RKM_AUTH_REQUIRED` is still `false`** (his opt-in — so signing out and still browsing is the FLAG, not a regression); the **second-launch request-log check on the phone** (A0's own gate, still his) and **A1's device eyeball** (the rows-from-disk check on the phone — ⚠ **A1 is now merged to `main` without that device round; if it is superseded, say so and I will pull it**); **Fill-as-default**; **tvOS go/no-go**; the Phase 0 device list; **A2** (`/api/library/home`, one call instead of six) then **A3** (the launch-budget gate) — both cheap, no Mac round.

**NEXT IN THE PLAN'S OWN ORDER: B2 — the native downloader** (`OfflineStore` + `OfflineDownloader` + the background-session delegate + cookie mirroring; 2–3 d, **first step that needs Xcode again**), then **B3** (the device's loopback server + the bridge). ⚠ B1 existing does **not** make the iPad play offline — it makes the *server* able to hand over a film, which is the prerequisite B2 was waiting for.
