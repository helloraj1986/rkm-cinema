## ▶ LATEST SESSION (2026-09-12) — SUBTITLE SELECTION FIX (found by the user's eyeball) ✅ FIXED, COMMITTED, PUSHED — ONE api+web REBUILD NEEDED (commit `5ba203a`, branch `feat/subtitles-opensubtitles`)
...
## ▶ LATEST SESSION (2026-09-12) — SUBTITLES, PHASES 0–4 OF 6 EXECUTED → **EYEBALLED 2026-09-12: one selection bug found and fixed in the block above** (branch: `feat/subtitles-opensubtitles`, commits `1db4215` (plan) → `4d08890` (P0) → `e27d28d` (P1) → `c7e4412` (P2) → `c29ce0b` (P3) → `9722d88` (P4); plan `SUBTITLES_OPENSUBTITLES_PLAN.md`. **Nothing is deployed yet — the api AND web changed, so this needs one rebuild + the user's eyeball. Phase 5 (docs/ADR-0005) is all that remains after that.**)
