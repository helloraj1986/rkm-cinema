## ▶ ✅ **THE UNSIGNED IPA, AND THE MAC'S THIRD VERB — ONE COMMAND BUILDS THE iPAD APP WITHOUT AN APPLE ID, AND SIDELOADLY SIGNS IT FROM RKM-HP** (2026-09-25) · branch **`feat/tvos-player`** · NEW `apple/scripts/build-ipa.sh`, `apple/scripts/test-build-ipa.sh`, `docs/UNSIGNED_IPA_PLAN.md` · changed `apple/scripts/mac-round.sh` (the `ipa` verb), `apple/README.md`, `apple/WORKFLOW.md`, `README.md`, `docs/ARCHITECTURE.md` §15 · ⚠⚠ **NOTHING under `backend/`, `frontend/` or `nginx/` changed — NO DEPLOY IS OWED**

**His ask, verbatim:** *"how can you look through rkm-cinema app, and make a plan to build unsigned ipa files for ios apps"*, then *"can we make this script part of rkm-cinema main command…where there are other commands available for building app for web and other stuffs"*.

⚠⚠ **AN UNSIGNED IPA IS NOT INSTALLABLE — IT IS AN INPUT TO A SIGNER.** iOS validates signatures at install; no setting or trick installs unsigned code. So the honest framing, which the plan's §0 leads with: the **7-day free-account clock is unchanged** (the profile travels inside the app whatever form it takes) and **what changes is which machine you need when it expires** — Sideloadly on RKM-HP, not the Mac and a cable. "Install it whenever you want" is NOT what this buys, and the plan says so rather than selling it.

**THE ONE COMMAND (Mac):** `./apple/scripts/mac-round.sh ipa [--release]` → `~/dev/rkm-cinema-dist/RKMCinema-unsigned.ipa`. It **`exec`s `apple/scripts/build-ipa.sh`**, which is the ONE implementation of the recipe — so it does **no `git pull`, no project generation, no build, no simulator and no signing**, and the verb is placed *before* the round's own pull so nothing of the round can run after the hand-off. Build: `xcodebuild … -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO …` → `Payload/` → `ditto -c -k --sequesterRsrc --keepParent`. **No Apple ID, no team, no provisioning profile, no `-allowProvisioningUpdates`** — the build never asks Apple for anything.

⚠ **WHY THE VERB IS ON THE MAC AND NOT IN `rkm-cinema.ps1` (his choice, from three options):** `rkm-cinema.ps1` is PowerShell 5.1 on **Windows** and `xcodebuild` exists only on **macOS**, so the Windows CLI cannot build an IPA at all. `apple/WORKFLOW.md` already states the boundary (*"GitHub is the only bridge to the Mac"*). ⚠ **The SSH alternative is recorded, deliberately rejected for now, not forgotten:** `.\rkm-cinema.ps1 apple ipa` building on the Mac and pulling the IPA back beside Sideloadly is the better UX, but it makes SSH a SECOND bridge where GitHub is the only one. Plan §4.9. ⚠ And a verb on the Windows CLI that silently did nothing would be worse than no verb — recorded so it is not "fixed" later by adding a no-op that looks like a feature.

⚠⚠ **TWO FAILURES PRODUCE AN IPA THAT LOOKS FINE, AND BOTH GET AN ASSERTION, NOT A COMMENT** (plan §4.5/§4.6): **(1)** a **simulator** build is *also* arm64, so `file` prints exactly what a device build prints — it packs, signs, and is refused at install with nothing naming the cause; the gate reads `LC_BUILD_VERSION` **`platform` and requires 2**, not 7 (awk only, no regex intervals — §7e rule 12 is that lesson). **(2)** a **stale signed `.app`** in a reused DerivedData would be packed under the name "unsigned"; hence a dedicated `-derivedDataPath`, the product path read from `-showBuildSettings` (⚠ never globbed — this repo paid for that already), and a `codesign` gate.

⚠⚠ **AND THE `codesign` ASSERTION HAD TO BE WIDENED — THE NARROW VERSION WAS A BUG.** Checking for the literal words *"not signed at all"* would have **blocked a perfectly good build**: on Apple Silicon the **linker ad-hoc signs arm64 automatically**, and ad-hoc signing **also creates `_CodeSignature/`**, so neither the string nor that directory is a valid test. It now rejects a **signing identity** (`Authority=` / `TeamIdentifier=`) and accepts `not signed at all` OR `Signature=adhoc`. ⚠ Note which way it errs: a wrong guess lets an ad-hoc app through (harmless — it is re-signed) instead of failing a good build with a confusing message. ⚠ The ad-hoc branch is **reasoned from the platform, not observed on a Mac** — said so in the script, in the plan, and in `apple-platform-clients`.

| | |
|---|---|
| **Files** | NEW `apple/scripts/build-ipa.sh` (8 numbered steps, each guard commented with the failure it prevents) · NEW `apple/scripts/test-build-ipa.sh` (the stub harness) · NEW `docs/UNSIGNED_IPA_PLAN.md` · `apple/scripts/mac-round.sh` (+ the `ipa` verb) · `apple/README.md` · `apple/WORKFLOW.md` · `README.md` (a new "The apps" section) · `docs/ARCHITECTURE.md` §15 (the new gate row) · `.gitignore` (+`.rkm-ipa-harness/`) |
| **Gates** | `test-build-ipa.sh` **14 cases / 0 failed** · `test-mac-round.sh` **10/10** · `check_md_links.py` **81 files, all resolve** · `bash -n` clean on all three scripts. ⚠ **pytest and vitest NOT run — correct, not an omission: no Python or TypeScript changed.** |
| **Falsified** | ⚠⚠ **The first falsification attempt was ITSELF WRONG, and that is the record.** Disabling all three guards at once turned only **A** and **G** red — breaking the `platform 2` *accept* case made B and C die at step 5c **before they could reach their own guard**, so two disabled checks looked like working ones. **A cascade is not a detection.** Redone one guard at a time: products-dir check → **B only**; identity rejection → **C only**; `platform 7` → **G only**; the forwarder's `exec` → **M, N**; the forwarder's *placement* (a `git fetch` inserted above it) → **M, N** |
| **⚠ NOT verified** | **Nothing Xcode-side.** The script has **never run against a real `xcodebuild`** — the harness proves the script's logic, the archive's structure (⚠ `unzip`/`shasum`/`grep` are never stubbed, so those assertions are evidence) and the hand-off. ⚠ And **Sideloadly accepting an unsigned IPA is not proved by anything in this repo** — it is the standard flow and well attested, but it is Phase 3, on his hardware. |
| **His answer that settled the design** | Free personal team (so unsigned + Sideloadly is the *only* useful shape — Ad Hoc/TestFlight are paid-only, ~AUD $149/yr) · iPadOS 26.x with **Developer Mode already on** |

⚠ Also corrected, while editing the same paragraph: `apple/WORKFLOW.md` §3 claimed `mac-round.sh` *"has not been run yet … written but unverified"* — **false since 2026-09-14**, when it ran on his Mac. Removed rather than left beside a new true statement.
...
⚠⚠ **THE VENDOR'S POPULARITY NUMBER WAS ALREADY ON THE WIRE AND SHOWN NOWHERE.** `download_count` is fetched,
parsed, decoded by BOTH clients and used to RANK results (criterion 8) — so the panel could say *which* row was
best but never *why*, and `used_count` (criterion 7) was rendered by the web panel and **not on tvOS**. Phase A
puts both facts in one payload — `settings`, `auto_language` and `auto` on `subtitle-search` — and gives the api
the rule that acts on them.

⚠⚠ **THE RULE IS TWO HALVES, AND THE SPLIT IS THE POINT: everything knowable LOCALLY answers before a single
request is made.** `auto_pick_blocked` (the switch · a stored choice — **including a per-title `Off`** · the
item's own subtitle in that language · the audio-language exclusion) is pure store/metadata; only a title that
passes all four reaches `auto_pick_shortfall` (the quota and the candidate) and the search. A test holds it with
`transport.count("/subtitles") == 0` — **a blocked title that still searched would be handing a metered account
to a rule that had already declined.**

⚠ **The candidate is the first row of the SAME ranking the panel draws** — our usage, then `download_count` —
filtered to the language and to non-SDH (his decision 4). That is what makes the badge and the applied subtitle
**incapable of disagreeing**, and it is why the badge says `Your pick before` when our usage put the row first
and `Most downloaded` only when popularity actually did.

⚠⚠ **THE STORE TRAP, AND IT WOULD HAVE BEEN INVISIBLE:** `_mutate` writes back whatever `load()` returned, so a
`settings` block that `load()` did not carry would be **ERASED by the next subtitle the user chose** — silently,
with nothing about that write looking wrong. `load()` now returns `prefs`, `usage` **and** `settings`, and a test
pins the survival.

⚠ **A DECLINE IS A 200, ALWAYS.** The viewer never asked for the download — the app did — so every refusal is
`decision` + `reason` from ONE vocabulary (`subtitles.AUTO_PICK_REASONS`), never a 500 the player must survive.
⚠ And **`quota_unknown` is a real answer he will meet**: without an OpenSubtitles login the allowance is only
reported ON a download, so his own decision (*0 or unknown → no attempt*) means an anonymous setup gets that
sentence instead of a hopeful attempt — the fix is `OPENSUBTITLES_USERNAME`/`PASSWORD` in `.env`, and the pane
will say so.
