## ▶ LATEST SESSION (2026-09-12) — SUBTITLES, PHASES 0–2 OF 6 EXECUTED ⏳ IN PROGRESS (branch: `feat/subtitles-opensubtitles`, commits `1db4215` (plan) → `4d08890` (P0) → `e27d28d` (P1) → `c7e4412` (P2); plan `SUBTITLES_OPENSUBTITLES_PLAN.md`. **API key received; Phase 3 next.**)
**User instruction:** *"proceed with next in rkm-cinema"* — the queued item was the scoped OpenSubtitles work. The branch was cut from an older `main`, so it was rebased onto current `main` (`251ec63`) first and force-pushed with an explicit `--force-with-lease` (only the plan doc + probe were on it). The user then registered an OpenSubtitles API key and pasted it into the repo `.env`, which unblocked the live half of Phase 2.

**Phase 0 (`4d08890`) — config plumbing + ONE .env parser.** Five settings declared on `Config` (`OPENSUBTITLES_API_KEY/USERNAME/PASSWORD/LANGUAGES/ENABLED`), read in `_load()`, routed into `.rkm.env` (api only), one api startup line, `.env.example` + live `.env` documented.
- **`has_opensubtitles()` keys off the API KEY alone.** A missing login degrades to the anonymous tier (5 downloads/24h per IP), never to "disabled" — key-only is a supported mode. `validate_required()` is untouched: a blank config must boot.
- **`backend/config/env_file.py` is now THE .env parser** for `render_config`, `config.settings` and `tools/rkm_common` (three copies before). The api's copy did a bare `partition("=") + strip()`, so a quoted value arrived WITH its quotes, and none tolerated a leading BOM. Proved old-vs-new on one quoted+BOM'd line: OLD `['\ufeffOPENSUBTITLES_PASSWORD', …]` → NEW `['OPENSUBTITLES_PASSWORD', …]`; `'p#ss w0rd'` → `p#ss w0rd`. (The plan's §4.1 trap is real — its own snippet starts with a BOM.)
- The render also prints whether OpenSubtitles is on and whether a login is configured, so "I pasted the key and nothing happened" is visible before the containers start.

**Phase 1 (`e27d28d`) — `services/opensubtitles.py` + 59 fake-transport tests** (no live calls in the suite). Search keyed `tmdb_id → imdb_id → title+year(+season/episode)`, imdb sent numeric (`tt0133093` → `133093`); empty results are `[]`, not an error; full typed taxonomy (`NotConfigured / AuthFailed / QuotaExhausted / RateLimited(Retry-After) / NoResults / UnsupportedFormat / TransportError`) defined WITH the client.
- **A `/download` POST is never retried** (it may already have been charged); idempotent GETs, including 5xx, are retried exactly once. The JWT is fetched lazily before a download, cached 12h, one re-login on expiry. The download `link` is pre-signed: fetched WITHOUT the `Api-Key`, never returned to a client, never logged.
- **Live search verified** (unmetered): tmdb_id and imdb_id both return 30 results for the same film, an episode keyed by title+year+season+episode returns 19 (his library's episodes carry NO provider ids — the title path is the one that matters), and a `hi` search returns Hindi results. One real download (91,617 bytes, plain `.srt`, UTF-8, LF) proved the byte path.
- ⚠ **Live finding:** the API's `attributes.format` is FREE TEXT, not an extension (`eng-sdh`, `eng-full`, `x265-heteam`, `23`). `format` now comes from `files[0].file_name`; the vendor label is kept as `vendor_format` for display only.

**Phase 2 (`c7e4412`) — delivery: `services/subtitles.py` + `item_path`/`refresh_item`/`upload_subtitle` on the provider.** The only code that writes to the media drive. Sidecar `<stem>.<lang>.srt` written atomically (tmp + `os.replace`, UTF-8, encoding normalised from UTF-8/UTF-16/cp1252); an existing same-language sidecar is **REUSED** (no download, the user's own file untouched); overwriting needs an explicit `replace_existing=True`; the **ITEM** is refreshed and a library scan can never be triggered (asserted by test); a non-subtitle payload (an HTML error page behind a 200) is refused rather than written; `resolve_active_track()` maps a stored identity to the CURRENT index (indices are positional) or to `None` — never to a different subtitle — with a 639-1/639-2 language normaliser (`en` vs `eng`, German `ger` ≠ `ge`).
- ⚠ **Live finding 1 — the upload endpoint is JSON, not multipart.** A hand-built multipart post is answered **415**; the server's own openapi declares `UploadSubtitleDto` (`Data` = base64). Found by reading the RUNNING server's `/api-docs/openapi.json` after the 415; implemented and re-verified (204 + track appears).
- ⚠ **Live finding 2 — indexing LAGS the delivery.** Reading tracks immediately after an upload can return the OLD list (the first live run reported `tracks: []` for a subtitle it had just uploaded). `_tracks()` now re-reads a bounded ~1 s before accepting an empty answer, and still reports empty honestly if that is the truth.
- ⚠ **Live finding 3 — `remaining` does not decrement for a repeat download of the SAME file** (three re-downloads of one file all reported `4`). Dedupe means we never do that, but the UI must say "as reported by the API", never present it as a hard daily count: an anonymous (key-only) setup has no `/infos/user` to cross-check with.
- **Verified live end-to-end** through `SubtitleService` against his own library (subtitle for "(500) Days of Summer", tmdb 19913, resolved by EXACT id — never a fuzzy match): the track appeared in `PlaybackInfo` and **the app's own VTT proxy URL served real content** — 119,258 chars of WEBVTT, first cue *"This is a story of boy meets girl."* The **upload fallback** is what ran (the sandbox has no `/data` mount — the fallback's own use case); the **sidecar path is unit-tested but NOT live-verified**, because only the api container can see the media mounts. ⚠ Everything the test added was removed again: the track deleted (204) and the item re-verified at **zero** external subtitle tracks, plus the temporary Jellyfin API key minted for the check deleted. An earlier cleanup pass missed its own leftovers (Jellyfin names an upload "English - SUBRIP - External", not the release name) — found, disclosed and cleaned in the same session.

**Gates:** **631 pytest, 0 failures** (Phase 0–2 added 100 tests) · ruff clean · docs link check 34 files / 7 links. ⚠ Correction to this record's first version: it reported `589 passed / 1 pre-existing failure (test_runtime_loader_merges_runtime_json)`. That failure was an ENVIRONMENT artefact of the sandbox shell exporting an EMPTY `JELLYFIN_API_KEY` (which the real-env passthrough applies over `runtime.json`); once the shell no longer carried that variable the test passed and the suite has been fully green since. The api's key comes from `.rkm.env`/`runtime.json`, never from a shell, so there is no production impact — but it IS a real (narrow) repo weakness worth a later look: an empty exported var silently overrides `runtime.json`.

**Next — Phase 3 (no key needed):** `services/subtitle_store.py` (atomic JSON beside the watchlist, corrupt-tolerant, usage increment, ranking) + the three endpoints (`GET /api/jellyfin/subtitle-search`, `POST /api/jellyfin/subtitle-select`, `POST /api/jellyfin/subtitle-disable`) + the additive `playback-info.preferred_subtitle`; contract 38 → **41 paths** with the snapshot and typed client regenerated in the same commit. Then Phase 4 (player panel + auto-apply) and Phase 5 (hardening, ADR-0005, docs). ⚠ Open decisions from the plan: §10.3 store = JSON sibling file (recommended, no migration) vs SQLite; §10.4 default languages (`en` only, or also `hi`/`ta`).
