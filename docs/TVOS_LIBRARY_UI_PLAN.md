...
| **Gates, on this commit** | `check-tvos-core.py` **466 checks / 0 failures** · `check-tvos-models.py` 113 keys / 17 endpoints · `check-apple-typecheck.sh` 21 portable files · `check-imports.py` 40 files · `check-tvos-members.py` 31 pairs, `--selftest` fires on all FIVE rules · `check-design-tokens.py` R1/R2/R3 · `check_md_links.py` 75 files · **`check-tvos-core.py --falsify`: all 16 of V's mutations and the repaired entry went red on the check each names, in the suite run of 2026-09-20** (that run also reported three PRE-EXISTING entries as rotten — two STALE, one WRONG — all three since repaired and each proved red by hand; a clean full-suite run over the repaired table is in flight and its verdict belongs in `PROGRESS.md`'s gate table) |
